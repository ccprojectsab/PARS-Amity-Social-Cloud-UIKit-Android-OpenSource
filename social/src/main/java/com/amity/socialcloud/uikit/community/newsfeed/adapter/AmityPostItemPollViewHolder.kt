                        ...
                        when {
                            Days.daysBetween(DateTime.now(), it.getClosedAt()).days > 0 -> {
                                ...
                                binding.statusTextView.text =
                                    AmityLocalisation.getString(R.plurals.amity_poll_status_closed_in_days, days, days)
                            }
                            Hours.hoursBetween(DateTime.now(), it.getClosedAt()).hours > 0 -> {
                                val hours = Hours.hoursBetween(DateTime.now(), it.getClosedAt()).hours
                                binding.statusTextView.text =
                                    AmityLocalisation.getString(R.plurals.amity_poll_status_closed_in_hours, hours, hours)
                            }
                            else -> {
                                val minutes = min(1, Minutes.minutesBetween(DateTime.now(), it.getClosedAt()).minutes)
                                binding.statusTextView.text =
                                    AmityLocalisation.getString(R.plurals.amity_poll_status_closed_in_minutes, minutes, minutes)
                            }
                        }
                    }
                    AmityPoll.Status.CLOSED == it.getStatus() -> {
                        binding.statusTextView.text = AmityLocalisation.getString(R.string.amity_poll_status_closed)
                    }
                }
            }
            .doOnNext {
                ...
                binding.voteCountTextView.text = AmityLocalisation.getString(R.plurals.amity_poll_vote_count, totalVoteCount, totalVoteCount)
            ...
            .doOnNext {
                if (!showFullContent && it.getAnswers().size > 2) {
                    binding.expandTextView.text = when (it.isVoted()) {
                        true -> AmityLocalisation.getString(R.string.amity_poll_expand_voted)
                        false -> AmityLocalisation.getString(R.plurals.amity_poll_expand, it.getAnswers().size - 2, it.getAnswers().size - 2)
                    }
